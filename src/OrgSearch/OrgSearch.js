...
import { updateField, getEntitiesById } from '../util';
      ...
      <div className="OrgSearch">
        ...
        <form onSubmit={this.handleSubmit}>
          ...
          <fieldset>
            ...
            {causes.map((cause) => (
              <div key={cause.id}>
                <label htmlFor={`cause-${cause.id}`}>{_.capitalize(cause.name)}</label>
                <input 
                  type="checkbox"
                  id={`cause-${cause.id}`}
                  name='causes'
                  value={cause.name}
                  onClick={() => this.flipCheckboxValue('causes', cause.name)}
                />
              </div>
            ))}
          </fieldset>
          ...
          <fieldset>
            ...
            {tags.map((tag) => (
              <div key={tag.id}>
                <label htmlFor={`tag-${tag.id}`}>{_.capitalize(tag.name)}</label>
                <input 
                  type="checkbox"
                  id={`tag-${tag.id}`}
                  name='tags'
                  value={tag.name}
                  onClick={() => this.flipCheckboxValue('tags', tag.name)}
                />
              </div>
            ))}
          </fieldset>
